...
c.LDAPAuthenticator.server_hosts = ['ldaps://ipa.daex.lan:636']
c.LDAPAuthenticator.bind_user_dn = 'uid=admin,cn=users,cn=accounts,dc=daex,dc=lan'
c.LDAPAuthenticator.bind_user_password = 'password'
c.LDAPAuthenticator.user_search_base = 'cn=users,cn=accounts,dc=daex,dc=lan'
...
c.LDAPAuthenticator.group_search_base = 'cn=groups,cn=accounts,dc=daex,dc=lan'
c.LDAPAuthenticator.group_search_filter = '(&(objectClass=ipausergroup)(memberOf={group}))'
c.LDAPAuthenticator.allowed_groups = ['cn=jupyterhub-users,cn=groups,cn=accounts,dc=daex,dc=lan']
